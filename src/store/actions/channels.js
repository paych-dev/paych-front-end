...
export const fetch_channels = () =>
  dispatch => {
    ...
    axios
      ...
      .then(response => dispatch(fetch_subscribed_channels_success(response.data)))
      .catch(error => {
        const newError = {
          id: new Date().getTime(),
    ...
    axios
      ...
      .then(response => {
        ...
        dispatch(fetch_own_channels_success(data, links));
      })
      .catch(error => {
        const newError = {
          id: new Date().getTime(),
...
const fetch_own_channels_success = (data, links) => ({
  type: actionTypes.FETCH_OWN_CHANNELS_SUCCESS,
  data: data,
  links: links
});
const fetch_subscribed_channels_success = channels => ({
  type: actionTypes.FETCH_SUBSCRIBED_CHANNELS_SUCCESS,
  payload: channels
});
...
export const fetch_own_channels = (next = undefined) =>
  dispatch => {
    dispatch(fetch_channels_start);
